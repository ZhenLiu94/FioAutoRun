...
if vms_config:
    #初始化跑fio的ssh类
    run_ssh_vms = RunSSH(fio_iops_bw_commands_vms, fio_lat_commands_vms, computes_config, vms_config, stor_mgt, watch_cluster_file_vms, runtime, poolname)
    connect_sshs_vms = []
    for i in vms_config.keys():
        connect_sshs_vms.append(run_ssh_vms.connect_ssh_mount(vms_config[i], mnt)) #需要带着mount
    # 创建json文件夹
    pre_config.create_dir(connect_sshs_vms, json_dir)
    # dd写文件的大小，单位是M，例如100就是100M，因为块是1M
    pre_config.dd_file(connect_sshs_vms, mnt_dir, dd_count)
    ...
    try:
        ...
        print e
        print("run fio failed.")

    for j in connect_sshs_vms:
        run_ssh_vms.close_ssh_umount(j, mnt) #需要带删除文件和umount
else:
    pass
...
if vms_config:
    ...
    get_result.move_remote_files(stor_mgt, watch_cluster_file_vms)
    for j in vms_config.keys():
        get_result.move_remote_files(vms_config[j], temp_json_dir)
else:
    pass
####################################
###生成计算节点excel
...
if computes_config:
    save_path = r"./"
    xlsx_name = "autotest_compute_multy"
...
if vms_config:
    save_path = r"./"
    xlsx_name = "autotest_vm_multy"
    workbook = DoExcel(save_path + xlsx_name + '.xlsx', xlsx_name)
    folder_name = workbook.get_location('vm')
    fio_path = './' + get_result.local_path + '/' + folder_name + '/'
    ceph_s_path = './' + get_result.local_path + '/ceph_s/' + folder_name + '/'
    #取数据
    lat_data, iops_bw_data = workbook.parse_json(fio_path, ceph_s_path)
    workbook.to_excel(lat_data, iops_bw_data, len(vm_ips))
    workbook.workbook.close()
else:
    pass
